...
func emit_dialogue(speaker: String, text: String, options: Array = []) -> void:
	emit_signal("dialogue_requested", speaker, text, options)
...
func _connect_debug_listeners() -> void:
	# Auto-log all signals when in debug mode
